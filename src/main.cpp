...
PYBIND11_MODULE(pyUnionFind, m) {
    pybind11::class_<DisjointSets>(m, "DisjointSets")
            .def(pybind11::init([] (int size) {return new DisjointSets(size); }))
            .def("find", &DisjointSets::find)
            .def("same", &DisjointSets::same)
            .def("unite", &DisjointSets::unite)
            .def("try_lock", &DisjointSets::try_lock)
            .def("unlock", &DisjointSets::unlock)
            .def("unite_index_locked", &DisjointSets::unite_index_locked)
            .def("unite_unlock", &DisjointSets::unite_unlock)
            .def("size", &DisjointSets::size)
            .def("rank", &DisjointSets::rank)
            .def("parent", &DisjointSets::parent)
            .def("__repr__", [](const DisjointSets& set) {std::stringstream stream; stream << set; return stream.str();});
}